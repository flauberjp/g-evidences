...
public class HistoryReflector {
  ...
  /**
   * Ideia geral ------------------------------------ Baixamos o repo local (nesse ponto do programa
   * o repo já existe remotamente).
   * <p>
   ...
   * Para cada projeto git, gera-se um arquivo texto com o seguinte nome: <diretório
   * git>_<e-mail>.txt, ex: cerberus-api_flauberjp@gmail.com
   * <p>
   * Para cada log cujo e-mail seja igual ao do usuário, incluimos o id do commit no arquivo.
   * Fazemos isso apenas se o id ainda não existir no arquivo.
   * <p>
   * Se a alteração tiver sido realizada no arquivo, efetuamos o commit dessa mudança
   * retroativamente, usando a data e hora do commit em analise.
   */
  public static void reflectHistory(List<GitDir> gitDirList, UserGithubInfo userGithubInfo) {
    try {
      ...
      for (GitDir gitDir : gitDirList) {
        if (gitDir.getAuthor()
            .equalsIgnoreCase(ProjetosGitDetectadosTableComponent.DESCONSIDERAR_HISTORICO)) {
          continue;
        }
        String fileNameWithItsPath =
            Util.getSolutionDirectoryIn83Format() + "/" + dir + "/" + new File(gitDir.getPath())
                .getName() + "_" + gitDir.getAuthor() + ".txt";
        createFileIfNotExist(fileNameWithItsPath);

        FileRepositoryBuilder builder = new FileRepositoryBuilder();
        Repository repo = builder.setGitDir(new File(gitDir.getPath() + "\\.git"))
        ...
        for (Iterator<RevCommit> iterator = log.iterator(); iterator.hasNext(); ) {
          ...
          commitCommand.setCommitter(new PersonIdent(
              new PersonIdent(userGithubInfo.getGithubName(), userGithubInfo.getGithubEmail()),
